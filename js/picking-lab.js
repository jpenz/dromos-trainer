...
(function () {
  ...
  const EXERCISES = [
    ...
    {
      id: "picked-dromos-line", category: "time", order: 6, title: "Picked dromos line",
    ...
    {
      id: "grouped-accents", category: "time", order: 7, title: "Greek grouped accents",
    ...
    {
      id: "tremolo-ladder", category: "time", order: 25, title: "Tremolo burst ladder",
    ...
    {
      id: "outside-pairs", category: "cross", order: 10, title: "Two-per-course crossing loop",
    ...
    {
      id: "mixed-crossings", category: "cross", order: 11, title: "Three-per-course mixed crossings",
    ...
    {
      id: "degree-window", category: "cross", order: 14, title: "Four-note degree windows",
    ...
    {
      id: "tactile-ab", category: "route", order: 22, title: "Horizontal ↔ tiered A/B",
    ...
    {
      id: "timbre-echo", category: "route", order: 23, title: "Course-timbre echo",
    ...
    {
      id: "triplet-grammar", category: "route", order: 20, title: "Alternate ↔ glide triplets",
      short: "Compare a strict down/up grid with a deliberate same-direction glide.", layout: "2nps", sequence: "tripletGrammar", count: 12, subdivision: 3,
    ...
    {
      id: "pick-legato", category: "phrase", order: 28, title: "Pick–hammer–pull–slide chain",
    ...
    {
      id: "mode-phrase-cell", category: "phrase", order: 29, title: "Dromos contour → home",
    ...
    {
      id: "arpeggio-arrival", category: "arpeggio", order: 32, title: "Triad arpeggio → next 3rd",
    ...
    {
      id: "open-course-penies", category: "time", order: 1, title: "Open-course penies primer",
      short: "The ta-ka clock on every open course before any left hand exists.", layout: "horizontal", sequence: "openCourses", count: 8,
      articulation: "picked-line", sourceIds: ["trigas-method", "pafranidis-sample", "berklee-online"],
      theory: "Every course of the instrument gets the same eight-stroke clock at the same volume: unequal course gauges are the first place evenness dies, so meet them with no left hand in the way. Optional control cue (an import, labelled): let the pick come to rest on the next string - institutional scale practice prescribes exactly this for pick control.",
    ...
    {
      id: "rhythm-formation-ladder", category: "time", order: 16, title: "Rhythm-formation ladder",
    ...
    {
      id: "pulse-accent-map", category: "time", order: 17, title: "16ths on the Greek pulse",
      short: "Continuous 16ths on one pitch; the accents are the dance's own group map.", layout: "horizontal", sequence: "pulseAccentMap", count: 16, subdivision: 4,
    ...
    {
      id: "skeleton-then-fill", category: "phrase", order: 18, title: "Skeleton, then fill",
    ...
    {
      id: "triplet-drive", category: "time", order: 19, title: "Triplet drive: glide families",
      short: "One pitch, continuous triplets, the two documented glide grammars.", layout: "horizontal", sequence: "tripletDrive", count: 12, subdivision: 3,
      articulation: "picked-line", sourceIds: ["pennanen", "papasolomontos-2017", "stahl-mandolin"],
      variants: [{ id: "ddu", label: "D-D-U" }, { id: "udd", label: "U-D-D" }, { id: "dud", label: "D-U-D · Stahl import" }],
    ...
    {
      id: "sextolet-glide", category: "cross", order: 21, title: "Sextolet glide over the crossing",
      short: "Six-note groups where the glide binds the course change.", layout: "2nps", sequence: "sextoletGlide", count: 12, subdivision: 3,
    ...
    {
      id: "counted-tremolo-groupings", category: "phrase", order: 26, title: "Counted tremolo groupings",
      short: "4+1, 6+1, 8+1 - then 4+2 and 6+4: bursts with a controlled landing stroke.", layout: "horizontal", sequence: "countedTremolo", count: 30, subdivision: 4,
    ...
    {
      id: "tremolo-entry-exit", category: "phrase", order: 27, title: "Tremolo entry & exit",
    ...
    {
      id: "irish-treble", category: "phrase", order: 30, title: "Treble cell (Irish import)",
      short: "Anchor note plus a D-U-D treble - a triplet cell from Irish plectrum pedagogy.", layout: "horizontal", sequence: "trebleCell", count: 8, subdivision: 4,
    ...
    {
      id: "era-register-contrast", category: "phrase", order: 31, title: "Two registers: sparse & dense",
    ...
    {
      id: "chunk-builder", category: "keychange", order: 35, title: "Chunk builder",
    ...
    {
      id: "ghammaz-pivot", category: "keychange", order: 36, title: "Pivot-note key change",
    ...
    {
      id: "band-key-arpeggio-circuit", category: "arpeggio", order: 37, title: "Band-key arpeggio circuit",
    ...
    {
      id: "sequence-ladder", category: "phrase", order: 38, title: "Imitation chain (mimisis)",
    ...
    {
      id: "skeleton-descent", category: "phrase", order: 39, title: "Skeleton descent (minore line)",
    ...
    {
      id: "instant-transpose", category: "keychange", order: 40, title: "Instant transpose",
    ...
    {
      id: "gap-click-pulse", category: "time", order: 41, title: "Gap click on the Greek pulse",
      short: "The click thins out - every subdivision, then group starts, then bar one only.", layout: "horizontal", sequence: "pulseAccentMap", count: 16, subdivision: 4,
      articulation: "picked-line", sourceIds: ["allingham-tempo", "measured-tempi", "trigas-method", "verwey-repp-background", "berklee-online"],
      ...
      evidence: "Sparse-click training at fixed tempo sits inside the documented tempo-management strategies (peer-reviewed: gradual increase and alternation validated as strategies, no step size validated); metronome placements on, off, and every other beat are institutional curriculum (Berklee Time and Rhythm, import); the pulse maps and tempo bands are the app's sourced rhythm model.",
      ...
      boundary: "Gap-click is a widespread teaching practice, not research-validated; Dromos applies it to the app's sourced Greek pulses and the stream is a generation."
    },
    {
      id: "full-neck-ladder", category: "route", order: 15, title: "Scale over the whole neck",
      short: "The same octave road rebuilt in every practical position, low frets to high and back.", layout: "box", sequence: "neckLadder", count: 32,
    ...
    {
      id: "arp-chunks", category: "arpeggio", order: 33, title: "Four-note chord chunks",
    ...
    {
      id: "loose-hand-ladder", category: "time", order: 5, title: "Loose hand, same clock",
    ...
    {
      id: "course-target", category: "cross", order: 9, title: "Name the course, land the course",
    ...
    {
      id: "through-stroke-landings", category: "time", order: 3, title: "Through-stroke landings",
      short: "Down THROUGH both strings of the course; the pick lands and rests on the next one.", layout: "horizontal", sequence: "throughStroke", count: 32,
      articulation: "picked-line", sourceIds: ["rosenberg-rest-stroke", "skamnelos-review", "pafranidis-sample"],
      variants: [{ id: "hold1", label: "Hold · 1 beat" }, { id: "holdHalf", label: "Hold · ½ beat" }, { id: "rebound", label: "Rebound" }],
      allStrings: true,
      theory: "An import wearing its badge: this is the gypsy-jazz/early-plectrum rest stroke, not Greek canon - no method in our surveyed Greek sources codifies it, and the Greek prescription is a perpendicular, minimal-depth tip stroke. As a TRAINING tool it does one thing: the stroke must travel through the course, not at it, so both strings speak and the attack depth stops varying. Two warnings from the sources themselves: Greek methods prescribe SHALLOW contact and describe deep digging (skapsimo) as a fault (Skamnelos review), and even Bickford calls always-resting inconvenient - so the landing is a calibration, never your default stroke. Design rationale, not a sourced claim: the landing may also teach the hand course spacing by touch. Whether you anchor a finger on the soundboard is a choice with a tradeoff, not a rule.",
      evidence: "The rest-stroke mechanic (wind up, through, land, free upstroke back) is taught in the Rosenberg Academy gypsy-jazz lessons; the slow, clear attack as the entry point of plectrum work is the Pafranidis sample. The per-course landing sequence and hold tiers are generated.",
      sourceLabel: "Rosenberg rest stroke (import) + Pafranidis attack", sourceHref: "https://rosenbergacademy.com/collections/lessons",
      steps: ["Wrist only, slow click: downstroke through both strings of the lowest course and let the pick REST on the next course for a full beat.", "The upstroke home is a free stroke. Four landings per course, then move to the next course, starting on a downstroke.", "Shorten the hold - one beat, half a beat, immediate rebound - without letting the stroke become a surface tap. On the last course, follow through the same distance into air."],
      listen: "Both strings of the course in every stroke, and the soft click of the pick arriving on the next course.",
      pass: "Self-scored: a full course cycle where every landing arrives and both strings sound, at each hold tier.",
      boundary: "An import labelled as such, used for attack depth training and not as performance style; the drill sequence is a Dromos generation."
    },
    {
      id: "monopenies-passes", category: "time", order: 4, title: "Monopenies: one direction at a time",
      short: "The same line three ways: all downstrokes, all upstrokes, then strict alternation.", layout: "horizontal", sequence: "monopenies", count: 24,
      articulation: "picked-line", sourceIds: ["pafranidis-sample", "praktiki-methodos-scan", "greek-teacher-first-lessons", "stahl-mandolin"],
      variants: [{ id: "open", label: "Open courses" }, { id: "fretted", label: "Fretted 1-2-3-4" }],
      allStrings: true,
      theory: "Isolate each stroke direction before recombining them: a pass of only downstrokes (the silent return glides back over the strings), a pass of only upstrokes, then the alternating pass. Documented in a Greek method scan and two independent teacher lessons; the causal story - that this builds each direction to equal strength - is design rationale, labelled as such.",
      evidence: "The down-only stage is documented in Greek print: Pafranidis opens the trichordo method with Exercise 1, thesi/downbeat, played 'slowly and cleanly'. The full down-up-alternate sequence appears in a method scan (corroborating but weak); the up-only pass is the weakest-documented of the three (teacher material) and ships as the symmetric counterpart; unidirectional drills are also in the Stahl mandolin method (import).",
      sourceLabel: "Greek method scan + teacher lessons + Stahl (import)", sourceHref: "https://www.youtube.com/watch?v=pPe1g2Y-qCM",
      steps: ["Play the traversal with EVERY note a downstroke; the return stroke travels silently above the strings.", "Repeat with every note an upstroke - same tempo, same volume, no cheating the click.", "Finish with strict alternation starting on a down. Each direction passes when a recording of it is as even as your alternating pass."],
      listen: "Whether the up-only pass sounds like the down-only pass; any direction that limps is the one to repeat.",
      pass: "Self-scored: on a recording, each single-direction pass matches the evenness of your alternating pass at this tempo.",
      boundary: "The direction-isolation sequence is the sourced pedagogy; this traversal, its passes, and the evenness comparison are Dromos generations, not copied exercises."
    },
    {
      id: "traversal-countdown", category: "cross", order: 8, title: "Traversal countdown",
      short: "Cross every course in order - four strokes per course, then two, then one.", layout: "horizontal", sequence: "traversalCountdown", count: 16,
      articulation: "picked-line", sourceIds: ["greek-teacher-first-lessons", "stahl-mandolin", "rembetiko-forum"],
      variants: [{ id: "four", label: "4 per course" }, { id: "two", label: "2 per course" }, { id: "one", label: "1 per course" }],
      allStrings: true,
      theory: "String hunting shrinks as the settle time per course shrinks: four strokes to find each course, then two, then a single stroke per course - a pure crossing chain. Every arrival on a new course is a downstroke (a documented plectrum rule, imported from Stahl). On the later tiers, try looking away from the right hand - a coaching prompt, not a graded condition.",
      evidence: "Repetition-per-course traversal is shown as a first lesson by the cited Greek teacher (video citation, noted at research time); down-on-new-string is documented in the Stahl mandolin method (import); community threads corroborate the traversal as common practice (folklore, corroborating only).",
      sourceLabel: "Greek teacher traversal + Stahl arrival rule", sourceHref: "https://www.youtube.com/watch?v=y_W53UIGzQM",
      steps: ["Walk every course low to high and back, four strokes per course, arriving on each new course with a downstroke.", "Drop to two strokes per course; the click does not change.", "Drop to one stroke per course - every note is now a crossing. If it smears, go back up one tier."],
      listen: "The moment of each course change: it should sound identical to the strokes around it.",
      pass: "Self-scored: the one-per-course tier runs the full traversal with no hesitation and no grazed neighbours at the current tempo.",
      boundary: "The countdown structure and traversal sequences are Dromos generations; the sources document the entry drill and the arrival rule, not this progression."
    },
    {
      id: "crossing-flip-stops", category: "cross", order: 12, title: "Crossing flip with full stops",
      short: "A two-course cell, a dead stop on the click, one note changes, re-enter exactly on time.", layout: "horizontal", sequence: "crossingFlip", count: 24,
      articulation: "picked-line", sourceIds: ["gilbert-polarity", "verwey-repp-background"],
      variants: [{ id: "three-one", label: "3+1 cells" }, { id: "one-one", label: "1+1 cells" }],
      theory: "Inside and outside crossings are two different movements, and adding or removing one note flips a cell from one to the other (a guitar-pedagogy import, labelled as such). The stops are ours: we designed them on the assumption that re-entry after silence is a distinct failure point - the chunking and continuation-timing literature motivates this but does not test it. Only one thing is scored: whether the first note after the silence lands on the click.",
      evidence: "Inside/outside polarity and the one-note flip are documented guitar pedagogy (Gilbert via Guitar World; import); one-note-per-string as the hardest case is likewise imported. Verwey (chunking) and Repp (synchronization-continuation) are BACKGROUND for the silence design, not proof this drill works.",
      sourceLabel: "Gilbert polarity (import) + background literature", sourceHref: "https://www.guitarworld.com/lessons/paul-gilbert-lesson-truth-about-inside-and-outside-picking-video",
      steps: ["Loop the cell four times with the click, watching which side of the crossing the pick approaches from.", "Mute dead ON a click beat; the click keeps going for two beats while the hand floats to neutral.", "Re-enter EXACTLY on the click with the flipped cell - one note added or removed - four times. Alternate cells; the first note after each silence is the whole exercise."],
      listen: "The first note after each silence: on the click, clean, on the named course - or the rep does not count.",
      pass: "Self-scored: four consecutive re-entries land on the click with no hesitation, in both cell polarities.",
      boundary: "The cells, stop placement, and flip logic are Dromos generations; the polarity concept is an import and the academic citations are background, not validation of this drill."
    },
    {
      id: "skip-thirds", category: "cross", order: 13, title: "Skip line in thirds",
      short: "The scale in broken thirds - every pair skips over a note, many pairs skip a course.", layout: "horizontal", sequence: "skipThirds", count: 12,
      articulation: "picked-line", sourceIds: ["leavitt-method", "avlonitis-101"],
      theory: "Skipping is crossing without a ladder: in broken thirds the pick leaves out the note between, so course changes arrive without the stepwise preparation a scale run gives you. The Berklee guitar method assigns string-skip etudes with the picking written out and strict attention demanded (an import wearing its badge); dexterity study is a declared Avlonitis category. Keep strict alternation - the skip is the variable, not the stroke pattern.",
      evidence: "String-skip etudes with mandatory notated picking are in the Leavitt method (import, open scan); graded dexterity is the declared frame of the Avlonitis collection. This broken-thirds realisation on the dromos is generated.",
      sourceLabel: "Leavitt skip etudes (import) + Avlonitis dexterity", sourceHref: "https://archive.org/details/modernmethodforg01leav",
      steps: ["Play the road in broken thirds - 1-3, 2-4, 3-5 - strict alternation, saying each pair's degrees out loud.", "Where a pair crosses a course, the skipped note is the trap: the pick must travel the extra distance in the same time.", "Reverse the line descending; then run it starting on the opposite stroke."],
      listen: "The skipped-course pairs: they must sound as even as the same-course pairs.",
      pass: "Self-scored: the full thirds line, up and down, from either starting stroke, with no smear at any skip.",
      boundary: "The sources support skip-etude and dexterity study; this broken-thirds line on the selected dromos is a Dromos generation, not a copied etude."
    },
    {
      id: "mair-density-ladder", category: "time", order: 24, title: "Density ladder at one slow tempo",
      short: "The click never moves: quarters down, 8ths down, 16ths alternate, 32nds alternate.", layout: "horizontal", sequence: "mairLadder", count: 30,
      articulation: "picked-line", sourceIds: ["mair-pick-technique", "pafranidis-sample", "manolopoulos-thesis"], subdivision: 4,
      theory: "Speed without chasing the metronome: the tempo stays put (the printed prescription is 50-60 BPM) and the note DENSITY doubles each bar - quarters, eighths, sixteenths, thirty-seconds. The first two passes are downstrokes only, the last two alternate. This is the one concrete printed tempo ladder found in plectrum pedagogy, a classical-mandolin import wearing its badge; by the last bar you are playing a measured tremolo without ever having sped up. Count each new density out loud for one bar before picking it - the Greek-native progression introduces 16ths and 32nds by counting first (Manolopoulos).",
      evidence: "The four-bar subdivision ladder at mm 50-60 with 10-20 repetitions is printed in Mair's classical-mandolin pick-technique study (import, labelled); the slow, clean single-direction entry point matches the Pafranidis sample's opening prescription.",
      sourceLabel: "Mair subdivision ladder (import)", sourceHref: "https://www.marilynnmair.com/articles/mandolin/2003/pick-technique/",
      steps: ["Set the click slow - the printed band is 50-60 BPM - and play one bar of quarter notes, all downstrokes, each one full and even.", "Double to eighths (still all down), then sixteenths (strict alternation), then thirty-seconds: four bars, one unchanged click.", "Loop the four bars. If the thirty-seconds bar smears, the ladder says stay - repeat the cycle, do not raise the tempo."],
      listen: "The click: it must feel identical under all four densities, and the 32nds bar should sound like a measured tremolo, not a scramble.",
      pass: "Self-scored: three consecutive four-bar cycles where every density lands inside the same unchanged click.",
      boundary: "The printed ladder is Mair's; this looping realisation on a bouzouki course and its pass structure are Dromos generations, not a copied page."
    },
    {
      id: "triad-ladder", category: "arpeggio", order: 34, title: "Triad ladder up the neck",
      short: "The same chord three times up the neck: root position, 1st inversion, 2nd inversion.", layout: "horizontal", sequence: "triadLadder", count: 18,
      articulation: "picked-line", sourceIds: ["polykandriotis-method", "weiss-triad-ladder", "trigas-method"],
      theory: "One chord lives in three neck regions: root position near the nut (1-3-5), first inversion above it (3-5-1), second inversion above that (5-1-3), then back down. One Greek method (Polykandriotis) organizes chords into three neck positions; this drill adopts that frame. The root-then-inversions climbing order is a guitar import, labelled as such. In Evolve with Band keys the ladder rebuilds in every band key, and the shapes become the landing points of the pivot cycle.",
      evidence: "Chords in three named positions with picked arpeggio exercises are documented in the Polykandriotis method (vol. 1); a graded chord/arpeggio track exists in the Trigas curriculum (catalogue level); the inversion-climb ordering itself is imported from guitar triad-ladder pedagogy (Weiss).",
      sourceLabel: "Polykandriotis three positions + Weiss ladder (import)", sourceHref: "https://polykandriotis.gr/wp-content/uploads/2020/05/vivlio1.pdf",
      steps: ["Play the root-position triad low on the neck as a picked arpeggio, naming the tones out loud: 1-3-5.", "Climb to the first inversion (3-5-1), then the second (5-1-3) - the same chord, higher window each time.", "Descend through all three shapes without stopping the pulse, then move to the next band key in Evolve."],
      listen: "The chord staying itself while the window climbs: three positions, one harmony.",
      pass: "Self-scored: all three shapes of the current key, up and down, with every tone named and no pulse bend at the shifts.",
      boundary: "The per-key ladder, tone labels, and pivot-cycle integration are Dromos generations; the three-position frame is the sourced Greek anchor and the inversion ordering is a labelled import, not a Greek prescription."
    }
  ];
  ...
  function buildSequence(exerciseId, baseNodes, pulse, firstStroke, variant) {
    ...
    if (exercise.sequence === "tripletDrive") {
      ...
      const target = baseNodes[0];
      if (!target) return [];
      const family = variant === "udd" ? ["U", "D", "DG"] : variant === "dud" ? ["D", "U", "D"] : ["D", "DG", "U"];
    ...
    if (exercise.sequence === "skipThirds") {
      // Broken thirds: pair starts accented; the skip itself is the drill.
      nodes = baseNodes.map((node) => cloneNode(node));
      return alternate(nodes, (index) => !!baseNodes[index].pairStart, firstStroke);
    }
    if (exercise.sequence === "mairLadder") {
      // One unchanged click, four densities (Mair import): quarters down,
      // 8ths down, 16ths alternate, 32nds alternate. At subdivision 4 the
      // durMults are 4 / 2 / 1 / 0.5 - the tempo itself never moves.
      const target = baseNodes[0];
      if (!target) return [];
      const passes = [
        { label: "quarters · all down", count: 4, durMult: 4, forced: "D" },
        { label: "8ths · all down", count: 8, durMult: 2, forced: "D" },
        { label: "16ths · alternate", count: 16, durMult: 1, forced: null },
        { label: "32nds · alternate", count: 32, durMult: 0.5, forced: null }
      ];
      nodes = [];
      passes.forEach((pass) => {
        for (let rep = 0; rep < pass.count; rep++) nodes.push(Object.assign(cloneNode(target), {
          phrase: pass.label, passStart: rep === 0, durMult: pass.durMult, forced: pass.forced
        }));
      });
      const line = alternate(nodes, (index) => !!nodes[index].passStart, firstStroke);
      return line.map((node, index) => nodes[index].forced
        ? Object.assign(node, { technique: nodes[index].forced, stroke: "down", crossing: null })
        : node);
    }
    if (exercise.sequence === "throughStroke") {
      // Rest-stroke trainer (import, labelled): landing downstroke with a
      // held rest on the next course, free upstroke home. Hold shrinks by
      // variant; the through-depth is the one graded variable.
      const hold = variant === "rebound" ? 1 : variant === "holdHalf" ? 1.5 : 2;
      nodes = [];
      baseNodes.forEach((course) => {
        for (let rep = 0; rep < 4; rep++) {
          nodes.push(Object.assign(cloneNode(course), { phrase: "land on next course", courseStart: rep === 0, durMult: hold, landing: true }));
          nodes.push(Object.assign(cloneNode(course), { phrase: "free return", durMult: 1 }));
        }
      });
      return nodes.map((node, index) => Object.assign(node, {
        order: index + 1,
        technique: node.landing ? "D" : "U",
        stroke: node.landing ? "down" : "up",
        accent: !!node.courseStart
      }));
    }
    if (exercise.sequence === "monopenies") {
      // Direction isolation: all-down pass (silent return), all-up pass,
      // then strict alternation. Crossing labels stay off forced passes -
      // they describe alternation, which those passes deliberately break.
      const passes = [["D", "monopenies · all down"], ["U", "all up"], [null, "diplopenies · alternate"]];
      nodes = [];
      passes.forEach(([forced, label]) => {
        baseNodes.forEach((node, index) => nodes.push(Object.assign(cloneNode(node), { phrase: label, passStart: index === 0, forced })));
      });
      const line = alternate(nodes, (index) => !!nodes[index].passStart, firstStroke);
      return line.map((node, index) => nodes[index].forced
        ? Object.assign(node, { technique: nodes[index].forced, stroke: nodes[index].forced === "D" ? "down" : "up", crossing: null })
        : node);
    }
    if (exercise.sequence === "traversalCountdown") {
      // Settle time per course shrinks 4 -> 2 -> 1; every arrival on a new
      // course is a downstroke (Stahl import), strokes alternate inside the
      // course block.
      const per = variant === "one" ? 1 : variant === "two" ? 2 : 4;
      nodes = [];
      baseNodes.forEach((course) => {
        for (let rep = 0; rep < per; rep++) nodes.push(Object.assign(cloneNode(course), { phrase: `${per} per course`, courseStart: rep === 0 }));
      });
      let step = 0;
      return nodes.map((node, index) => {
        if (node.courseStart) step = 0; else step++;
        const down = step % 2 === 0;
        return Object.assign(node, { order: index + 1, technique: down ? "D" : "U", stroke: down ? "down" : "up", accent: !!node.courseStart });
      });
    }
    if (exercise.sequence === "crossingFlip") {
      // Cell x4, two beats of clicked silence, the one-note-flipped cell x4.
      // Silent nodes keep the alternation clock honest; only the re-entry
      // is scored, and it is self-scored.
      const lower = baseNodes.filter((node) => node.cell === "lower");
      const upper = baseNodes.filter((node) => node.cell === "upper");
      if (!lower.length || !upper.length) return [];
      const small = variant === "one-one";
      const cellA = small ? [lower[0], upper[0]] : lower.slice(0, 3).concat(upper.slice(0, 1));
      const cellB = small ? [lower[0], upper[0], lower[1] || lower[0]] : lower.slice(0, 2).concat(upper.slice(0, 1));
      nodes = [];
      const pushBlock = (cell, label) => {
        for (let rep = 0; rep < 4; rep++) cell.forEach((node, index) => nodes.push(Object.assign(cloneNode(node), { phrase: label, blockStart: rep === 0 && index === 0 })));
        for (let rest = 0; rest < 4; rest++) nodes.push(Object.assign(cloneNode(cell[0]), { silent: true, phrase: "silence · hand floats" }));
      };
    ...
    if (exercise.sequence === "triadLadder") {
      // Root, 1st, 2nd inversion climbing then descending; inversion labels
      // ride the nodes (app supplies placements + labels).
      nodes = baseNodes.map((node) => cloneNode(node));
      return alternate(nodes, (index) => !!baseNodes[index].inversionStart, firstStroke).map((node, index) =>
        Object.assign(node, { phrase: baseNodes[index].inversionLabel || "ladder" }));
    }
