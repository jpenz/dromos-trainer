...
| **FR-65** | Universal answer-first page guidance | DONE | — | `js/page-guides.js`, `js/app.js`, `css/styles.css` | Every workspace and meaningful sub-exercise starts with the result the learner is trying to achieve, a visible three-step path using the controls' exact labels, an observable success condition, and a plain-language reason. Definitions and basic screen instructions remain one optional level deeper. “Show me where” moves keyboard/touch focus to a real first control, and the guide updates with the selected ear drill, Solo activity, style section, key, scale, instrument, and Changes Gym mode. |
...
| **FR-67** | Source-bounded Picking Lab | DONE | — | `js/picking-lab.js`, `js/practice.js`, `js/app.js`, `index.html`, `css/styles.css` | A top-level page sequences nine original, instrument-aware exercises through even attack, Greek grouped accents, tremolo bursts, isolated/mixed course crossings, Pennanen horizontal/tiered and timbre comparisons, pick/legato articulation, and triad-to-next-3rd arrival. Each exercise provides exact events, animated fretboard positions, a listening goal, pass test, direct evidence link, explicit non-transcription boundary, first-stroke/subdivision controls, tap-to-inspect, decisive Stop, and a three-clean-pass tempo gate. |
...
| **MI-34** | The learner's answer and complete three-step path are never hidden in a disclosure. Every configured guide has exactly three actions, a success condition, a reason, at least two definitions, and a first-action target that exists in the shipped interface. Sub-exercises that change the learner's job resolve to distinct guides. | ✅ — pure guide self-test, target-existence audit, and responsive source regression |
...
| `js/picking-lab.js` | FR-67, FR-68 | MI-36, MI-37 | `PickingLab.selfTest()` + category/source/stroke/accent/tremolo/ornament and loop/key/position-plan contracts in `npm test` |
