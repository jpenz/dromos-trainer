...
| **FR-21** | Picking strokes & crossings | DONE | — | `js/practice.js` | Strict alternate strokes marked ↓/↑ with full downstroke/upstroke control labels; each string change classified inside/outside per MI-11 and colour-coded on the path |
...
| **FR-27** | Practice curriculum path | DONE | — | `index.html`, `js/app.js` | Changes → Recall → Melody → Map → Comp → Solo → Picking → Pulse → Study remains visible and each step opens the matching practice area with a concrete cue |
...
| **FR-53** | Named local player profiles | DONE | — | `js/profiles.js`, `js/coach.js`, `js/app.js` | Up to eight validated profiles on this device keep stable settings, instrument, ear progress, and sing-back locks independently; Dre defaults to tetrachordo bouzouki, coach tokens are namespaced per player, and the UI explicitly avoids claiming cloud authentication |
...
| **FR-63** | Private microphone sing-back | DONE | — | `js/pitch-lab.js`, `js/profiles.js`, `js/app.js`, `index.html`, `css/styles.css` | After revealing a Melody note, the player can hear it, explicitly grant microphone access, select a browser-exposed input, and sing it in any octave. The page distinguishes a different scale degree from flat/sharp tuning, shows cents, clarity, stability, and a one-second in-tune hold, saves only pass/attempt counts per local profile, never records/uploads audio, and stops every input track on Stop, navigation, profile switch, or question change |
| **FR-64** | Matrix Chord Path | DONE | — | `js/chord-path.js`, `js/app.js`, `css/styles.css` | Clicking any Matrix chord opens an inline four-question path for the selected instrument: validated 0–15-fret voicings, chord-tone arpeggio cells and a lawful triad/seventh enhancement, single-course scale-only lines into a chosen R/3/5/7 target, exact progression-evidenced successors, and chord-specific relative/parallel/key doors. Every result recalculates when key, scale, chord depth, target, or tuning changes. |
| **FR-65** | Universal answer-first page guidance | DONE | — | `js/page-guides.js`, `js/app.js`, `css/styles.css` | Every workspace starts with the result and one visible first action using the controls' exact labels. The complete three-step path, listening reason, success condition, definitions, and basic screen instructions remain one tap deeper so the actual instrument controls stay in the first viewport. “Show me where” moves keyboard/touch focus to a real first control, and the guide updates with the selected ear drill, Solo activity, style section, key, scale, instrument, and Changes Gym mode. |
| **FR-66** | Source-bounded tactical example index | DONE | — | `js/tactical-examples.js`, `js/toolkit.js`, `js/melody-harmony.js`, `js/app.js`, `index.html`, `css/styles.css` | Learn contains a categorized index of concrete, transposing examples for every Solo Toolkit concept plus Pennanen's horizontal/tiered neck comparison. Each entry states the source-supported observation, current key/dromos/instrument setup, exact notes or functions, one compact selected-instrument route at fret 15 or lower, ordered actions, listening goal, pass test, source link, and the boundary that the generated drill is not a transcription. Toolkit and Melody claims link directly to the relevant example and then back into the matching Solo practice. |
| **FR-67** | Source-bounded Picking Lab | DONE | — | `js/picking-lab.js`, `js/practice.js`, `js/app.js`, `index.html`, `css/styles.css` | A top-level page sequences ten original, instrument-aware exercises through even attack, Greek grouped accents, tremolo bursts, isolated/mixed course crossings, Pennanen horizontal/tiered/timbre comparisons, a route-aligned strict-alternate versus glide/sweep triplet A/B, pick/legato articulation, and triad-to-next-3rd arrival. Each exercise provides exact ↓/↑/full-word motion, animated pick and fretboard positions, selected-key/dromos/chord theory, a listening goal, pass test, direct evidence link, explicit non-transcription boundary, first-stroke/subdivision controls, tap-to-inspect, decisive Stop, and a three-clean-pass tempo gate. |
| **FR-68** | Loop/evolve picking practice engine | DONE | — | `js/picking-lab.js`, `js/audio.js`, `js/app.js`, `index.html`, `css/styles.css` | Every Picking Lab exercise can set 40–180 BPM and run as 1, 2, 4, or 6 identical loops or evolve through instrument-verified neck positions, circle-of-fourths keys, or both. A one-bar grouped count-in and cancel-safe metronome share the note scheduler; the run roadmap exposes every coming stage, playback updates the key/fretboard in place, Stop clears the complete run, and an evolved run resumes from its final key/position. |
| **FR-69** | Recoverable reference audio | DONE | — | `js/audio.js`, `js/app.js`, `index.html`, `css/styles.css` | Playback uses an awaitable AudioContext unlock during the user's tap, reports running/suspended/blocked state, times out instead of hanging on WebKit's unresolved resume, and offers Test sound beside both global and Picking sound choices. Harmony and scored ear work use the self-hosted sampled piano or warm-key fallback; Picking independently defaults to a short paired-course bouzouki attack so articulation does not inherit a sustained chord voice. |
| **FR-70** | Provenance-ranked bouzouki mastery spine | DONE | — | `js/bouzouki-knowledge.js`, `js/picking-lab.js`, `js/app.js`, `index.html`, `css/styles.css` | Picking Lab sequences 13 original exercises through ta–ka attack, coordination, route, phrase language, and performance transfer. Every exercise declares articulation, authority-ranked sources, a rights boundary, a listening goal, and a pass gate; note-by-note picking is the articulated-line default and tremolo is an explicit sustain branch. |
...
| **FR-75** | Authored harmonic phrases and practice lenses | WIP | — | `docs/PROGRESSION_TRAINING_STRATEGY.md` | Replace FR-74's interim duration-by-event-count rule with explicit per-progression event durations, resolution edges, evidence grades, and separate Hear/Comp/Slow Target/Song Time transformations. A turnaround may resolve across its declared repeat boundary; a practice slowdown must never rewrite the underlying song progression. |
...
| **MI-32** | Sing-back grades the pitch class in any octave, not a prescribed vocal register. A steady wrong degree never counts as in tune; a correct pitch must stay within 25 cents with sufficient periodic clarity for one continuous second. Silence creates no failed attempt. The microphone graph is input-only and is never connected to the audio destination. | ✅ — synthetic detector, scoring, shell, and profile regressions |
| **MI-33** | A Matrix Chord Path never predicts repertoire from a scale table. “Can follow” requires exact adjacent chords in `Modes.PROGRESSIONS`; connector notes remain inside the displayed fixed collection; a pivot door holds the same pitch-class chord, a recolour door holds the root, and the ii–V–I role-change door is available only when major I becomes the next major key's ii one whole step lower. Every displayed connector has a one-course 0–15-fret realization on every supported tuning. | ✅ — exhaustive 12-tonic × 5-mode × 7-degree × 2-depth path audit plus five-tuning connector checks |
| **MI-34** | The learner's answer and exact first action are never hidden in a disclosure. Every configured guide has exactly three actions, a success condition, a reason, at least two definitions, and a first-action target that exists in the shipped interface; the remaining guidance stays one tap away. Sub-exercises that change the learner's job resolve to distinct guides. | ✅ — pure guide self-test, target-existence audit, and responsive source regression |
| **MI-35** | A named musician, scholar, method, or tradition may support only the observation the cited source actually documents. Every generated tactical sequence must identify itself as an original Dromos drill—not a transcription, quotation, historic phrase, or claim that the named player performed those notes. All generated notes must remain finite and recalculable across the 12 tonics and five encoded dromoi; mode-specific examples must be visibly unavailable outside their supported maps. | ✅ — source/link/schema self-test plus exhaustive 12-tonic × 5-dromos generation regression |
| **MI-36** | A picking source may support a technique family, motor-structure observation, or curriculum sequence without supplying Dromos's exact exercise. Picking events therefore remain explicitly app-authored; strict alternate plans never repeat a stroke, grouped accents follow the selected pulse's group starts, tremolo bursts preserve their declared 2+4+6+8 counts, ornaments distinguish picked from left-hand attacks, and every generated position comes from the selected tuning. | ✅ — `PickingLab.selfTest()` plus app-shell/offline contracts and all-tuning path coverage inherited from `Practice.selfTest()` |
| **MI-37** | Repetition and variation are separate jobs. Loop preserves tonic and position exactly. Evolve changes only the selected dimension: practical positions follow deduplicated playable paths for the selected tuning; keys follow a deterministic circle of fourths; both combines those two declared movements. The count-in sounds once at run start, metronome accents reuse the selected Greek grouping, and stopping invalidates every remaining stage, callback, note, and click. | ✅ — `PickingLab.selfTest()` loop/key/position plans plus audio/app-shell cancellation contracts |
| **MI-38** | A glide mark is musical behavior, not decoration: DG/UG may appear only on an actual course change, the selected first stroke is honored, and the compared route preserves pitch order while changing stroke grammar. Audio unlock must resolve true or false within a bounded interval so no control remains indefinitely in a starting state. | ✅ — route-aware `PickingLab.selfTest()` plus audio/app-shell contracts and phone/tablet browser verification |
| **MI-39** | Every Picking Lab exercise declares one articulation contract and resolves to known provenance. A `picked-line` event gives every displayed note one alternating D/U attack; tremolo appears only in an explicit `tremolo-sustain` exercise; community evidence is never the sole authority for a music rule; and no commercial exercise, notation, recording, or artist phrase is represented as ingested app content. | ✅ — `BouzoukiKnowledge.selfTest()` + `PickingLab.selfTest()` + source/articulation/ordering contracts in `npm test` |
...
| **NFR-08** | Microphone analysis is local, transient, and permission-bound. Raw samples remain in one reusable in-memory buffer, no audio is recorded or sent to APIs/analytics, input tracks stop when the exercise is not active, and the UI names browser/iPad routing limitations without promising a specific physical device. | DONE |
...
| `js/pitch-lab.js` | FR-63 | MI-32 | `PitchLab.selfTest()` + sung-range synthetic pitch, octave-equivalence, wrong-degree, and silence regressions in `test/pitch-lab.test.js` |
...
| `js/tactical-examples.js` | FR-66 | MI-35 | `TacticalExamples.selfTest()` + every-tool link audit and exhaustive 12-tonic × 5-dromos note/instruction regression in `npm test` |
| `js/bouzouki-knowledge.js`, `js/picking-lab.js`, `js/audio.js` | FR-67, FR-68, FR-69, FR-70, FR-71 | MI-36, MI-37, MI-38, MI-39, MI-40 | `BouzoukiKnowledge.selfTest()` + `PickingLab.selfTest()` + category/source/articulation/stroke/accent/tremolo/ornament, mastery-order, route-aligned glide, loop/key/position, audio-unlock, envelope, and separate-voice contracts in `npm test` |
...
| `js/fretboard.js` | FR-02, FR-03, FR-10, FR-11, FR-16, FR-57, FR-72 | MI-10, MI-12, MI-27, MI-41 | geometry contract + Solo two-zone full-neck source/browser regression + `Modes.selfTest()` |
